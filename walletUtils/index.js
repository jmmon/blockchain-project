...
const getCompressedPublicKey = (compactPubKey) =>
...
const deriveKeysFromMnemonic = async (mnemonic) => {
	...
	const hexPublicKeyCompressed = getCompressedPublicKey(hexPublicKeyCompact);
	const testingDecompress = decompressPublicKey(hexPublicKeyCompressed);
	console.log(
...
const signTransaction = (privateKey, txDataHashBuffer) => {
	...
	try {
		...
		const [r, s] = splitSignature(signedData);
...
const decryptAndSign = async (
	...
	console.log('-- decryptAndSign result:', { txData });
...
const verifySignature = (txDataHash, publicKey, signature) => {
	console.log(`fn verifySignature`, { txDataHash, publicKey, signature });

	// h == txDataHash
	// Q == their public key?
	publicKey = decompressPublicKey(publicKey); // decompress, hope this fixes it!
	console.log(`--`, {decompressedPubKey: publicKey});
